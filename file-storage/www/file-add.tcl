ad_page_contract {
	...
	if {[exists_and_not_null file_id] && ![exists_and_not_null folder_id]} {
	    ...
	    set doc_type_id [db_string get_doc_id "select distinct doc_type from cr_revisions where item_id=$file_id LIMIT 1" -default 0]
...
if {$status=="chekd_in"} { 
	db_dml update_state "update cr_items set publish_status ='ready', status_updated_by=$user_id where item_id=$file_id"
}
...
if {![info exists file_id]} {
	ad_form -extend -form {
   	{doc_type_id:text(im_category_tree) {label "[_ intranet-core.Document_Type]"} {custom {category_type "Intranet Document Type"} } }
   	}
} else {
	ad_form -extend -form {
   	{doc_type_id:text(hidden) {value $doc_type_id} }
   	}
}
...
#ad_return_complaint 1 "$upload_file====$this_file_id===$folder_id === $tmpfile===$user_id === $this_title===$description===$package_id==$mime_type"
	fs::add_file \
	    -name $upload_file \
	...
	db_dml update_doc_type "
			update cr_revisions
			set doc_type =$doc_type_id
			where item_id=$this_file_id	
	"
