...
ad_proc -public fs::before_unmount {
    -package_id
    -node_id
} {
    Create root folder for package instance
...
# Author      : Subbu
# Date        : 26th Apr 2010
# Description : To create physical folder structure
     set folder_type "$object_type"
     if {$project_id !=""} {
        set base_path [im_filestorage_base_path $folder_type $project_id]
        set err_msg [im_filestorage_create_folder_ajaxfs "$base_path$folder_path/" $pretty_name]
     }
# --------------End----------------------------
...
# Subbu@VMC-Start
# To rename physical folders
    set folder_type "$object_type"
    if {$new_path != $old_path} {
    set base_path [im_filestorage_base_path $folder_type $project_id]
    file rename -- $base_path$old_path $base_path$new_path
    }
# Subbu@VMC-End
...
#ashok pathak, added project_id and folder_path to add_file.
ad_proc -public fs::add_file {
   ...
   #ad_return_complaint 1 "$package_id"
    if {[parameter::get -parameter "StoreFilesInDatabaseP" -package_id $package_id -default 0]} {
    set indbp "t"
    ...
    if {$no_callback_p} {
        set revision_id [fs::add_version \
            ...
            ns_log notice "fs::add_version***********"
        #ashok pathak ,   add project_id and folder_path to add_version
        set revision_id [fs::add_version \
                 -name $name \
                 -tmp_filename $tmp_filename \
                 -package_id $package_id \
    ...
    #ashok pathak commented notification and callback
...
#Ashok Pathak added project_id and folder_path
...
#ashok pathak   , add project_id and folder_path to cr_import_content procs   
    set revision_id [cr_import_content \
             -item_id $item_id \
             -storage_type $storage_type \
             -creation_user $creation_user \
...
# Author      : Subbu
# Date        : 26th Apr 2010
# Description : ajaxfs - create physical folder

ad_proc -public im_filestorage_create_folder_ajaxfs {folder folder_name} {
    Create a new folder
} {
    ns_log notice "im_filestorage_create_folder"
     if { [catch {
         exec /bin/mkdir -p $folder
         exec /bin/chmod ug+w $folder
