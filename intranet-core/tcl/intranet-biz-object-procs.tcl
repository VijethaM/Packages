...
ad_proc -public im_group_member_component { 
    ...
    if {$object_type=="im_company"} {
        append header_html "
        <td class=rowtitle align=middle>[_ intranet-core.Email]</td>
        <td class=rowtitle align=middle>[_ intranet-core.Phone]</td>
        "
    }
...
append body_html $name
    ...
    set phone_num [db_string s "select cell_phone from users_contact where user_id=$user_id" -default ""]
	append body_html "$profile_gif</td>"
    if {$object_type=="im_company"} {
            append body_html "<td>$email</td><td>$phone_num</td>"
        }
	if {$show_percentage_p} {
	    append body_html "
		  <td align=middle>
