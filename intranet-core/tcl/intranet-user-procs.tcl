...
ad_proc im_customer_select_multiple { select_name { defaults "" } { size "6"} {multiple ""}} {
    set bind_vars [ns_set create]
	...
	where
		u.user_id = gm.member_id
		and gm.group_id = $customer_group_id
	order by lower(im_name_from_user_id(u.user_id, $name_order))
    "
    return [im_selection_to_list_box -translate_p "0" $bind_vars category_select $sql $select_name $defaults $size $multiple]
} 
...
#vijetha@vmc 19/03/2013 added below   procs to get list of users without Senior Manger and po-admin 
ad_proc im_employee_select_multiple_wo_sm { select_name { defaults "" } { size "6"} {multiple ""}} {
    set bind_vars [ns_set create]
    set employee_group_id [im_employee_group_id]
    set name_order [parameter::get -package_id [apm_package_id_from_key intranet-core] -parameter "NameOrder" -default 1]
    set sql "
	...
	where
		u.user_id = gm.member_id
		and gm.group_id = $employee_group_id
		and  gm.member_id not in (select member_id from group_distinct_member_map where group_id in ([im_profile_po_admins],[im_profile_senior_managers],[im_director_group_id]))
	order by lower(im_name_from_user_id(u.user_id, $name_order))
    "
    return [im_selection_to_list_box -translate_p "0" $bind_vars category_select $sql $select_name $defaults $size $multiple]
}
...
#vijetha@vmc 23/03/2013 added below  procs get list of PM,Directors, Senior Project Managers
ad_proc im_dir_pm_spm__select_multiple { select_name { defaults "" } { size "6"} {multiple ""}} {
set bind_vars [ns_set create]
    set pm_group_id [im_pm_group_id]
...
where
        u.user_id = gm.member_id
        and gm.group_id in ([im_director_group_id],[im_pm_group_id],[im_sr_pm_group_id])
...
ad_proc im_user_efficiency_component { date user_id} {
    Returns a formatted HTML showing the status of notifications  for the current user
} {
   set params ""
   set result [ad_parse_template -params $params "/packages/intranet-core/www/users/user_efficiency"]
   return $result
