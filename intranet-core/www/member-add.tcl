...
#set company_name [db_string a "select company_name from im_companies where company_id=$object_id"]
...
#ad_return_complaint 1 "$object_type"
if {$object_type=="im_company"} {
	set selection_type "Customer"
	set company_name [db_string a "select company_name from im_companies where company_id=$object_id" -default ""]
	set page_title "Add new member to Company $company_name"
} else {
	set page_title "[_ intranet-core.lt_Add_new_member_to_obj]
}
...
#
if {$object_type=="im_company"} {
	set selection_type "Customers"
	set employee_select [im_customer_select_multiple user_id_from_search "" 12 multiple]
	 
} else {
	set selection_type "Employees"
...
<table cellpadding=0 cellspacing=2 border=0>
  <tr> 
    <td class=rowtitle align=middle>$selection_type</td>
  </tr>
  <tr> 
    <td>
$employee_select
