...
ad_proc im_employee_info_component { employee_id return_url {view_name ""} } {
 ...
 #    }
    ...
    set not_display_list ""
    if {$view_name=="employees_view"} {
    	append not_display_list "and c.column_name not in ('Availability %','Currency','Hourly Cost','Social Security nr','Salary','Social Security','Insurance','Other Costs','Salary Period','Salaries per Year','Termination_Date','Vacation Days per Year','Vacation Balance From Last Year')"
    }
    set column_sql "
	...
	where	c.view_id=:view_id $not_display_list
	order by sort_order"
