...
set edit_buttons [list]
 if {[info exists task_id]} {
	...
	lappend edit_buttons { "Create Timesheet Task" "ok" }
}
ad_form \
    -name task \
...
if {1 && $form_mode !="display"} {
   ...
   if {[info exists task_id]} {
   	set page_title [_ intranet-timesheet2-tasks.Edit_Timesheet_Task]
	set base_component_title [_ intranet-timesheet2-tasks.Edit_Timesheet_Task]
	
   } else {
   	set page_title [_ intranet-timesheet2-tasks.New_Timesheet_Task]
	set base_component_title [_ intranet-timesheet2-tasks.New_Timesheet_Task]
   }
...
ad_form -extend -name task -on_request {
	...
	if {$previous_task_status==""} {
    	...
    	if {(($current_task_status=="Work In Progress" || $current_task_status=="Closed") && ($previous_task_status!="Closed" && $is_first_task!=1))} {
