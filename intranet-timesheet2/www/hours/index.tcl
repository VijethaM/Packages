...
set gr_id_sel [db_list abc " select group_id from groups where group_name in ('Cxo', 'P/O Admins') "]
set usr_groups [db_list grp "select group_id from group_distinct_member_map where member_id=:current_user_id"]
set grp_match 0
foreach gr_id_sels $gr_id_sel {
foreach usr_group $usr_groups {
if {$gr_id_sels ==$usr_group} {set write_p 1}
}
}
# ad_return_complaint 1 "$write_p"
...
for { set i $start_day } { $i < 7 } { incr i } {
    if { $i ==0 } { append header_days_of_week "[_ intranet-timesheet2.Monday] " }
    if { $i ==1 } { append header_days_of_week "[_ intranet-timesheet2.Tuesday] " }
    if { $i ==2 } { append header_days_of_week "[_ intranet-timesheet2.Wednesday] " }
    if { $i ==3 } { append header_days_of_week "[_ intranet-timesheet2.Thursday] " }
    if { $i ==4 } { append header_days_of_week "[_ intranet-timesheet2.Friday] " }
    if { $i ==5 } { append header_days_of_week "[_ intranet-timesheet2.Saturday] " }
    if { $i ==6 } { append header_days_of_week "[_ intranet-timesheet2.Sunday] " }
}
...
for { set current_date $first_julian_date} { $current_date <= $last_julian_date } { incr current_date } {
    ...
    if {$write_p} {
    set hours_url [export_vars -base "new" {user_id_from_search {julian_date $current_date} show_week_p is_from_home_page return_url project_id project_id}]
    ...
    if {$column_ctr == 1 && !$timesheet_entry_blocked_p && $show_link_log_hours_for_week_p } {
        append hours "<br>
                <a href=[export_vars -base "new" {user_id_from_search {julian_date $current_date} {show_week_p 1} {is_from_home_page 1} return_url}]
        ...
        set no_unconfirmed_hours [db_string as "select sum(hours) from im_hours where user_id=$current_user_id and day>=to_date(:current_date, 'J') and day<=to_date(:current_date, 'J') and is_submited='f'" -default 0]
                ...
                # ns_log notice "There are no unconfirmed hours: [info exists hash_conf_object_id($julian_date)]"
                        ...
                        if { $no_wf_cases > 0 } {
#                            ad_return_complaint 1 "$no_wf_cases $hours"
                    ...
                    set html "$html<br><br>" 
                    if { $confirm_timesheet_hours_p } {
                        append html "[lang::message::lookup "" intranet-timesheet2.ToConfirm "To Submit"]:&nbsp;"
    ...
    append html "<br>
        <a href=[export_vars -base "week" {{julian_date $current_date} user_id_from_search is_from_home_page} ]
    ...
    if {$confirm_timesheet_hours_p} {
        ...
        set start_date [db_string start_date "select to_date(:start_date_julian_wf, 'J')"]
        set end_date [db_string start_date "select to_date(:end_date_julian_wf, 'J')"]
        set no_unconfirmed_hours [get_unconfirmed_hours_for_period $current_user_id $start_date_julian_wf $end_date_julian_wf]
        set no_unconfirmed_hours [db_string as "select count(*) from im_hours where user_id=$current_user_id and day>=:start_date and day<=:end_date and is_submited='f'" -default 0]
        ...
        # ad_return_complaint 1 "$no_unconfirmed_hours $start_date $end_date"
        if {$confirm_timesheet_hours_p && 0 < $no_unconfirmed_hours  } {
        # ad_return_complaint 1 "$no_unconfirmed_hours"
        set base_url_confirm_wf "/intranet-timesheet2/hours/hours_submit"  
        set conf_url [export_vars -base $base_url_confirm_wf { {user_id $user_id_from_search} {start_date_julian $start_date_julian_wf} {end_date_julian $end_date_julian_wf } {is_from_home_page 1} return_url}]
        set button_txt [lang::message::lookup "" intranet-timesheet2.Confirm_weekly_hours "Submit hours for this week"]
...
<a href=\"index?[export_url_vars user_id_from_search]&date=\$ansi_date\">
  <font color=#414042>\[_ intranet-timesheet2.$prev_month_name] </font>
</a>"
set next_month_template "
<a href=\"index?[export_url_vars user_id_from_search]&date=\$ansi_date\">
  <font color=#414042>\[_ intranet-timesheet2.$next_month_name]</font>
</a> 
<font color=#414042>&gt;</font>"
